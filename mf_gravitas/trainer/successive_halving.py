...
def train_evaluate(model: HalvingGridSearchCV, **kwargs):
    ...
    level_dfs = [df.loc[df['param_algo_id'].isin(s)] for (_, df), s in
                 zip(relevant_runhistory.groupby('iter'), survivors)]
    ...
    rank = pd.DataFrame.from_dict({'rank': terminated.index, 'algo_id': terminated[
        'param_algo_id']})
