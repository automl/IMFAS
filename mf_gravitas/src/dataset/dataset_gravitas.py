...
class Dataset_Gravity(Dataset):
    ...
    def __init__(
        ...
        :param n_competitors: number of datasets that are compared against. Notice that n_competitors = 2
        ...
        if deselect > 0:
            self.deselected = self._reduce_algo_space(
                removals=deselect, k=topk, mode=deselection_metric
            )
            # redoit all again, so that the getitem never sees these algos
            self.preprocess_with_known_deselection(
                self.deselected, dataset_meta_features, learning_curves, algorithms_meta_features
            )
    ...
    def __getitem__(self, item):
        ...
        # TODO move k to init
        # TODO seedit
        item_compareset = random.choices(list(set(range(self.nD)) - {item}), k=self.n_competitors)
    ...
    def preprocess(
        ...
        learning_curves = {
            k: {int(k1): v1 for k1, v1 in v.items()} for k, v in learning_curves.items()
        }
    ...
    def preprocess_with_known_deselection(
        self, deselected, dataset_meta_features, learning_curves, algorithms_meta_features
    ):
        """
    ...
    def _preprocess_learning_curves(self, algorithms_meta_features, learning_curves):
        ...
        self.algo_learning_curves = {k: {} for k in algorithms_meta_features.keys()}
    ...
    def _preprocess_meta_features(self, dataset_meta_features):
        ...
        df = pd.DataFrame(list(dataset_meta_features.values()), index=dataset_meta_features.keys())
        ...
        numerical_df = pd.DataFrame(
            numerical_df, columns=self.columns_numerical, index=dataset_meta_features.keys()
        )
        ...
        categorical_df = pd.DataFrame(
            categorical_df,
            columns=self.enc_cat.get_feature_names(),
            index=dataset_meta_features.keys(),
        )

        self.datasets_meta_features_df = pd.concat(
            [numerical_df, categorical_df, binary_df], axis=1
        )
    ...
    def _preprocess_dataset_properties(self, learning_curves, dataset_meta_features):
        ...
        self.dataset_learning_properties = {d: Datum() for d in learning_curves.keys()}
        ...
        for d in learning_curves.keys():
            ...
            datum.convergence_avg_topb = np.mean([algotup[3] for algotup in datum.ranking[:b]])
    ...
    def _reduce_algo_space(self, k=10, removals=5, mode="skew"):
        ...
        performance_baseline = df.max(axis=1) - df[np.argsort(df, axis=1) >= k_inv].mean(
            axis=1,
        )
        ...
        for r in range(removals):
            ...
            if mode == "skew":
                ...
                deselected_algo = pd.Series(
                    {k: calc_pdf_integral0(v, n_bins=100) for k, v in performances.items()}
                ).argmin()
    ...
    def plot_convergence90_time(self, normalized=True):
        ...
        for dataset_id, dataset_curves in self.raw_learning_curves.items():
            ...
            if normalized:
                ...
                title = (
                    "Unconditional Time Budget until 90% Convergence\n"
                    "normalized by available budget"
                )
            else:
                unconditional_90time = [
                    curve.convergence90_time for curve in dataset_curves.values()
                ]
