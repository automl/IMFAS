...
def pipe_train(cfg: DictConfig) -> None:
    ...
    if model_type.startswith('masif_C_transformer'):
        if 'full_lc2global' in model_opts:
            cfg.wandb.group = cfg.wandb.group + '_flc'
            cfg.wandb.tags[0] = cfg.wandb.tags[0] + ' F LC'

    if cfg.trainer.trainerobj._target_ == 'masif.trainer.sh_scheduler.SHScheduler':
