...
from margret.utils import MLP


# marius proposal: prepend the dataset meta features to the sequence.

class margretTransformerMLP(nn.Module):
    def __init__(
            self,
        ...
        """This model class defines the computational macro graph of the margret model.
        This variant passes the entire learning curve tensor to the transformer encoder jointly! """
        super(margretTransformerMLP, self).__init__()
    ...
    def forward(self, learning_curves: torch.Tensor, mask: torch.Tensor,
        ...
        if isinstance(self.transformer_lc, margretGuidedAttentionTransformerEncoder):
