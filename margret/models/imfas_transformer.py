...
from margret.utils.positionalencoder import PositionalEncoder
from margret.models.margret_transformer_guided_attention import margretGuidedAttentionTransformerEncoder


class AbstractmargretTransformer(nn.Module):
    def __init__(
            self,
            n_algos: int,
            n_fidelities: int,
            ...
            decoder: nn.Module,
            transformer_lc: margretGuidedAttentionTransformerEncoder,
            device: str = "cpu",
            model_opts: list[str] = []
    ):
        """
        Abstract margret Transformer models. Here I propose two types of Transformer models. The first one is to consider
        ...
        """
        super(AbstractmargretTransformer, self).__init__()
...
class margretBaseTransformer(AbstractmargretTransformer):
    """
    This implementation follow the margret's LSTM's implementation, switching the
    ...
    def __init__(
        ...
        super(margretBaseTransformer, self).__init__(n_algos, n_fidelities, dataset_metaf_encoder, algo_metaf_encoder,
...
class margretHierarchicalTransformer(AbstractmargretTransformer):
    """
    An margret Trasnforemr with hierarchical architecture. For the input, we first flatten them to perform local attention
    ...
    def __init__(
            ...
            decoder: nn.Module,
            transformer_lc: margretGuidedAttentionTransformerEncoder,
            transformer_algo: margretGuidedAttentionTransformerEncoder,
            device: str = "cpu",
            model_opts: list[str] = []
    ):
        self.EOS = torch.tensor(0, device=torch.device(device))  # End of Sequence

        super(margretHierarchicalTransformer, self).__init__(
    ...
    def to(self, device):
        ...
        super(margretHierarchicalTransformer, self).to(device)
...
class margretMLPTransformer(margretHierarchicalTransformer):
    def __init__(
            self,
            n_algos: int,
            n_fidelities: int,
        ...
        self.EOS = torch.tensor(0, device=torch.device(device))  # End of Sequence
        super(margretMLPTransformer, self).__init__(
...
class margretCrossTransformer(margretHierarchicalTransformer):
    """
    An margret Transformer with hierarchical architecture. For the input, we first flatten them to perform local attention
    operation with self.transformer_lc. Then in the second stage, we perform a global attention operation with
    self.transformer_algo_encoder
    """
    ...
    def __init__(
        ...
        super(margretCrossTransformer, self).__init__(n_algos, n_fidelities, dataset_metaf_encoder, algo_metaf_encoder,
