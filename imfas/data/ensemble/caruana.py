...
def ensemble(loss_table, init_size=1, replacement=True, max_iter=100):
    """
    ...
    if init_size == 1:
        ...
        y_hat_avg = np.array([y_hats[mod] for mod in init_ensemble]).mean(axis=0)
    ...
    model_weights = pd.DataFrame(index=ensemble_loss.index, columns=y_hats.keys())
...
if __name__ == "__main__":
    ...
    # FIXME: @Tim please use relatve paths for the csv's
