...
class Dataset_Joint_TaskswiseRanking(Dataset_Join_Dmajor):
    ...
    def __init__(
        ...
        assert lc_dims[0] == meta_dataset_dims[0], (
            f"Number of datasets from lc and meta_dataset must equal,"
            f"However, they are {lc_dims[0]} and {meta_algos_dims[0]}"
            f"respectively."
        )
        assert lc_dims[-1] == meta_algos_dims[0], (
            f"Number of algorihms from lc and meta_algos_dims must equal,"
            f"However, they are {lc_dims[0]} and {meta_algos_dims[0]}"
            f"respectively."
        )
        ...
        if self.is_test_set and split is None:
            raise ValueError("If the dataset is test set, it must contain the information of trainer sets")
