...
from imfas.data import Dataset_Join_Dmajor
from imfas.models.rank_mlp import ActionRankMLP
from imfas.trainer.rank_trainer import train_rank
from imfas.util import seed_everything, train_test_split
...
def pipe_train(cfg: DictConfig) -> None:
    ...
    train_set = Dataset_Join_Dmajor(
        meta_dataset=dataset_meta_features, lc=lc_dataset, split=train_split
    )

    test_set = Dataset_Join_Dmajor(
        meta_dataset=dataset_meta_features, lc=lc_dataset, split=test_split
    )

    # wrap with Dataloaders
    train_loader = DataLoader(
        train_set, batch_size=cfg.train_batch_size, shuffle=cfg.shuffle, num_workers=cfg.num_workers
    )

    test_loader = DataLoader(
        test_set, batch_size=cfg.test_batch_size, shuffle=cfg.shuffle, num_workers=cfg.num_workers
    )
