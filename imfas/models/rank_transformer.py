...
class RankTransfromer(nn.Module):
    ...
    def forward(self, init_feature: torch.Tensor, context: torch.Tensor):
        """
        transformer forward pass. we first embedds context to a sequence and then attach init_features in front of it

        Args:
            init_feature: initialized features generated by MLP with size [B, D_model]
        ...
        net_input = torch.concat([torch.unsqueeze(init_feature, 1), context])
...
class RankTransformer_Eesemble(nn.Module):
    def __init__(
            self,
            input_dim: int = 107,
            algo_dim: int = 58,
    ...
    def _build_network(self):
        ...
        self.transformer_network = RankTransfromer(
    ...
    def forward(self, dataset_meta_features, fidelities):
        ...
        transformer_D = self.transformer_network(init_features=shared_D, context=fidelities)
